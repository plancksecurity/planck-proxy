...
def cleanup():
    ...
    if settings['dts'] is not None:
        ...
        if logpath is not None:
            for a in glob(os.path.join(logpath, "*.eml")):
                attachments += [a]
        dbgmail("As requested via activated Return Receipt here's your debug log:",
                settings['dts'], "[DEBUG LOG] pEp Gate @ " + socket.getfqdn(), attachments)
    ...
    if settings['DEBUG']:
        dbg(
            f"Debug mode, will keep the logged output messages in the work_dir {c(logpath, 6)}", pub=False)
    else:
        try:
...
def dbgmail(msg, rcpt=None, subject="[FATAL] pEp Gate @ " + socket.getfqdn() + " crashed!", attachments=[]):
    ...
    mailcontent += (msg + "<br>" + ("=" * 80) +
                    "<br><br>" if len(msg) > 0 else "") + settings['htmllog']
    ...
    if len(attachments) > 0:
        for att in attachments:
            ...
            mailcontent += "\n\n--pEpMIME\n"
            mailcontent += "Content-Type: application/octet-stream; name=\"" + \
                os.path.basename(att) + "\"\n"
            mailcontent += "Content-Disposition: attachment; filename=\"" + \
                os.path.basename(att) + "\"\n"
    ...
    for subthing in thing:
        if isinstance(subthing, str):
            ...
            for k, v in subthing.items():
                ...
                ret += c(str(k).rjust(colwidth), 6) + c(" | ", 5) + \
                    str(v).replace("\n", "\n" +
                                   (" " * colwidth) + c(" | ", 5)) + "\n"
...
def keysfromkeyring(userid=None):
    ...
    for r1 in q1:
        q2 = db.execute(
            "SELECT * FROM subkeys WHERE primary_key = ?;", (r1[1],))
        subkeys = []
        for r2 in q2:
            subkeys += [str(r2[0])]
        ...
        q3 = db.execute(
            "SELECT tpk, secret FROM keys WHERE primary_key = ?;", (r1[1],))
        for r3 in q3:
            sqkeyfile = ("sec" if r3[1] == True else "pub") + \
                "." + r1[0] + "." + r1[1] + ".key"
            open(sqkeyfile, "wb").write(r3[0])
            cmd = [sq_bin, "enarmor", sqkeyfile, "-o", sqkeyfile + ".asc"]
            p = Popen(cmd, shell=False, stdin=PIPE, stdout=PIPE,
                      stderr=PIPE)  # stderr=STDOUT for debugging
            ret = p.wait()
...
def decryptusingsq(inmail, secretkeyglob):
    ...
    ret = ""
    patt = re.compile(
        r"-----BEGIN PGP MESSAGE-----.*?-----END PGP MESSAGE-----", re.MULTILINE | re.DOTALL)
    ...
    for p in pgpparts:
        ...
        for secretkey in glob(secretkeyglob):
            ...
            cmd = [sq_bin, "decrypt", "--recipient-key",
                   secretkey, "--", tmppath]
