    ...
    while os.path.isfile(settings['lockfilepath']) and locktime < settings['locktimeout']:
        ...
        if lockpid.isdigit() and int(lockpid) > 1:
            try:
                ...
                dbg("Lock held by dead PID " + lockpid +
                    ", removing lockfile", pub=False)
                cleanup()
                lockpid = None
            else:
                dbg("Lock held by active PID " + lockpid + ", killing in " +
                    str(settings['locktimeout'] - locktime) + "s", pub=False)
        else:
            dbg("Lockfile doesn't contain any numeric PID [" + str(
                lockpid) + "]. Removing file", pub=False)
            cleanup()
        locktime += 1
        sleep(1)
...
def enable_dts(message):
    ...
    global settings
    dts = getmailheaders(
        message.msg['inmail'], "Disposition-Notification-To")  # Debug To Sender

    if len(dts) > 0:
        addr = dts[0]
        dts = "-".join(re.findall(re.compile(
            r"<?.*@(\w{2,}\.\w{2,})>?"), addr))
...
def check_key_reset(message):
    ...
    if (kw in message.msg['inmail'] for kw in keywords) and settings['mode'] == "encrypt" and message.us['addr'] in settings['reset_senders'] and message.them['addr'] not in settings['reset_senders']:
        dbg("Resetting key for " +
            message.them['addr'] + " in keyring " + message.us['addr'])

        for kw in keywords:
            message.msg['inmail'] = message.msg['inmail'].replace(kw, "")

        keys_db_location = os.path.join(str(settings['workdir']), '.pEp')
        deletekeyfromkeyring.delete_key(
            message.us['addr'], message.them['addr'], keys_db_location)
...
def addr_domain_rewrite(message):
    ...
    forwarding_map_path = os.path.join(
        settings['home'], settings['forwarding_map'])
    rewrite = jsonlookup(forwarding_map_path, message.us['addr'], False)
    if rewrite is not None:
        dbg("Rewriting our address from " +
            c(message.us['addr'], 1) + " to " + c(rewrite, 3))
        ...
        if rewrite is not None:
            dbg("Rewriting domain of message from " +
                c(ourdomain, 3) + " to " + c(rewrite, 1))
...
def init_logging(message):
    ...
    logfilename = os.path.join(
        logpath, "in." + settings['mode'] + ".original.eml")
...
def print_keys_and_keaders(message):
    ...
    dbg(c("┌ Headers in original message (as seen by non-p≡p clients)",
...
def check_recipient_pubkey(pEp, message):
    ...
    if theirkey == False:
        dbg("No public key for recipient " +
            c(message.them['addr'], 3) + ", p≡p won't be able to encrypt this time")
        theirpepid = pEp.Identity(message.them['addr'], message.them['addr'])
    else:
        dbg(c("Found existing public key for recipient ", 2) +
            c(message.them['addr'], 5) + ":\n" + prettytable(theirkey))
        ...
        message.them['keyname'] = theirkeyname
        message.them['keyaddr'] = theirkeyaddr
        message.them['keyfpr'] = theirkeyfpr

        theirpepid = pEp.Identity(theirkeyaddr, theirkeyname)

    message.them['pepid'] = theirpepid
...
def check_sender_privkey(message):
    ...
    if ourkey == False:
        dbg("No private key for our address " +
            c(message.us['addr'], 3) + ", p≡p will have to generate one later")
        ourkeyname = ourkeyaddr = ourkeyfpr = None
    else:
        dbg(c("Found existing private key for our address ", 2) +
            c(message.us['addr'], 5) + ":\n" + prettytable(ourkey))
        # TODO: this doesn't support multiple UID's per key, we should figure out the most recent one
...
def set_own_identity(pEp, message):
    ...
    if message.us['keyname'] is None and message.us['keyaddr'] is None and message.us['keyfpr'] is None:
        ...
        if ourname is None:
            ...
            dbg(c("No matching name found", 1) + " for address " +
                c(message.us['addr'], 3) + ", using de-@'ed address as name: " + c(ourname, 5))
        else:
            dbg("Found name matching our address " +
                c(message.us['addr'], 3) + ": " + c(ourname, 2))
        ...
        if ourname is not None and ourname != message.us['keyname']:
            dbg(c("Name inside existing key (" +
                message.us['keyname'] + ") differs from the one found in username.map (" + ourname + "), using the latter", 3))
            ...
            # redundancy needed since we can't use myself'ed or update'd keys in pEp.Message.[to|from]
            ourpepid = pEp.Identity(
                message.us['keyaddr'], message.us['keyname'])
    ...
    try:
        if settings['mode'] == "encrypt":
            ...
            if len(message.msg['src'].from_.username) == 0:
                tmp = pEp.Identity(
                    message.msg['src'].from_.address, message.msg['src'].from_.address)
                message.msg['src'].from_ = tmp
                dbg("Added missing username to src._from: " +
                    repr(message.msg['src'].from_))
            ...
            # Magic-string "NOENCRYPT" found inside the message
            elif "NOENCRYPT" in message.msg['src'].longmsg + message.msg['src'].longmsg_formatted and b in settings['noencrypt_senders']:
                dbg(c(
                    f"Found magic string 'NOENCRYPT' so not going to encrypt this message {settings['DEBUG']}", 1))
                dst = message.msg['src']
                dst.longmsg = dst.longmsg.replace("NOENCRYPT", "")
                dst.longmsg_formatted = dst.longmsg_formatted.replace(
                    "NOENCRYPT", "")
                ...
                if len(settings['EXTRA_KEYS']) == 0:
                    ...
                    dbg("└ with extra key(s): " +
                        ", ".join(settings['EXTRA_KEYS']))
        ...
        if settings['mode'] == "decrypt":
            # TODO: store some sort of failure-counter (per message ID?) to detect subsequent failures then fallback to sq, then forward as-is
            pepfails = False
            if not pepfails:
                ...
                tmp = decryptusingsq(message.msg['inmail'], os.path.join(
                    settings['work_dir'], "sec.*.key"))
                dst, keys, rating, flags = pEp.Message(
                    tmp[0]), tmp[1], None, None
                dbg(c("Decrypted in", 2), True)
            ...
            if str(rating) == "have_no_key":
                keys_path = os.path.join(
                    settings['home'], settings['keys_dir'])
                dbg(c("No matching key found to decrypt the message. Please put a matching key into the " +
                    c(keys_path, 5) + " folder. It will be sent encrypted to the scanner", 1))
                # exit(7)
...
def add_routing_and_headers(pEp, message):
    ...
    if settings['mode'] == "encrypt":
        nextmx = jsonlookup(
            nextmx_path, message.them['pepid'].address[message.them['pepid'].address.rfind("@") + 1:], False)

    if settings['mode'] == "decrypt":
        nextmx = jsonlookup(
            nextmx_path, message.us['pepid'].address[message.us['pepid'].address.rfind("@") + 1:], False)
    ...
    if settings['DEBUG']:
        dbg("Optional headers:\n" +
            prettytable(message.msg['dst'].opt_fields), pub=False)
...
def deliver_mail(message):
    ...
    dbg("From: " + ((c(message.msg['src'].from_.username, 2)) if len(
        message.msg['src'].from_.username) > 0 else "") + c(" <" + message.msg['src'].from_.address + ">", 3))
    dbg("  To: " + ((c(message.msg['src'].to[0].username, 2)) if len(
        message.msg['src'].to[0].username) > 0 else "") + c(" <" + message.msg['src'].to[0].address + ">", 3))
