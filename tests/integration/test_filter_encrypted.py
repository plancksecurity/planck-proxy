...
def test_filter_enc_good(
    test_dirs, extra_keypair, collect_email, settings_file, test_settings_dict
):
...
@pytest.mark.parametrize("collect_email", ["basic.enc.eml"], indirect=True)
def test_filter_av_fails(
    test_dirs, extra_keypair, collect_email, settings_file, test_settings_dict
):
    test_settings_dict["scan_pipes"] = [
